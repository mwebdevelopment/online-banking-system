...
$id=  mysqli_real_escape_string($connect, $_REQUEST['customer_id']);
$sql="SELECT * FROM `customer` WHERE id=$id";
$result=  mysqli_query($connect, $sql) or die(mysqli_error());
$rws=  mysqli_fetch_array($result);
?>
<?php
                        $delete_id=  mysqli_real_escape_string($connect, $_REQUEST['customer_id']);
                        if(isset($_REQUEST['submit2_id'])){
                            $sql_delete="DELETE FROM `customer` WHERE `id` = '$delete_id'";
                            $sql_drop="DROP TABLE passbook".$delete_id;
                            mysqli_query($connect, $sql_delete) or die(mysqli_error());
                            mysqli_query($connect, $sql_drop) or die(mysqli_error());
